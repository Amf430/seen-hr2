...
/* ── ألغاها الموظف، أم عجز عنها جهازه؟ ──

   ⚠️ هذا التمييز هو ما يقرّر: نُلغي العملية أم نمضي فيها.

   إلغاء الموظف قرارٌ منه: ضغط «تسجيل الحضور» ثم رفض شاشة الوجه/البصمة
   يعني أنه عدل — فتُلغى العملية ولا يُكتب شيء. أما جهاز بلا قفل شاشة أو
   متصفح لا يدعم WebAuthn فعجزٌ لا ذنب له فيه، ومنعُه من التسجيل يتركه
   واقفاً داخل الفرع بلا حيلة.

   `declined-enroll` إلغاءٌ أيضاً: هي رفض شاشة ربط الجهاز أول مرة.
   و`verify-failed` ليست منها — بصمة لم تُطابَق قد تكون إصبعاً خاطئاً أو
   عطلاً في المستشعر، لا قراراً بالإلغاء. */
const BIO_USER_CANCELLED = new Set(['cancelled', 'declined-enroll']);

export const bioUserCancelled = (res) =>
  !!res && !res.ok && BIO_USER_CANCELLED.has(res.reason);
