...
/* ٢٤ ← ١٨ بإخراج الإعدادات، ← ١٧ بدمج مصدرَي الحضور، ← ١٥ بإخراج أرشيف
   ...
   ⚠️ «يومي» وجهةٌ يومية، وجدول المناوبات وجهة تشغيلية مستقلة لمسؤولها وHR. */
eq("⚠️ الأدمن ١٨ رابطاً — بينها تقرير الحضور وجدول المناوبات", 18, count("admin"));
...
/* ١٦ ← ١٥ بدمج «تسجيل حضوري» في الرئيسية، ← ١٦ بإضافة «يومي»، ← ١٧ بجدول المناوبات */
...
for (const role of ROLES) {
  ...
  if (role === 'manager') {
    ...
    eq('«التقارير» عند الأدمن ٥ — كلها تُقرأ ولا تُعدَّل', ['التقارير=5'], big);
  } else {
...
ok('تقرير الحضور للأدمن وحده لأنه يعيد استخدام القراءة غير المقيّدة نفسها',
   canOpen('attendance-reports', 'admin') &&
   !canOpen('attendance-reports', 'manager') && !canOpen('attendance-reports', 'employee'));
